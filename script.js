...
firebase.database().ref('ultimaSenha').on('value', (snapshot) => {
  ...
  if (ultimaSenhaFirebase === ultima.texto) {
    // Já está sincronizado, não precisa fazer nada.
    return;
  }
  ...
  tentarCentralizarBotaoQuandoExistir(ultima.texto);
});




function centralizarBotao(textoSenha, tentativas = 0) {
  if (tentativas > 10) {
    console.warn(`Não foi possível centralizar o botão ${textoSenha} após 10 tentativas.`);
    return;
  }
  ...
  if (!encontrou) {
    setTimeout(() => centralizarBotao(textoSenha, tentativas + 1), 200);
  }
}
...
function esperarSegundoKey(tipo) {
  ...
  document.addEventListener('keydown', segundaLetra, { once: true });  // <-- evita acumular listeners
}

function tentarCentralizarBotaoQuandoExistir(textoSenha, tentativas = 0) {
  if (tentativas > 10) {
    console.warn(`Não foi possível centralizar o botão ${textoSenha} após 10 tentativas.`);
    return;
  }
  ...
  if (!encontrou) {
    setTimeout(() => {
      tentarCentralizarBotaoQuandoExistir(textoSenha, tentativas + 1);
    }, 200);
  }
}
