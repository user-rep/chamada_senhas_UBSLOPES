...
function processarChamadaSenha(idColuna, textoSenha, tipo) {
  const isPreferencial = tipo === 'preferencial';
  const numeroSenha = parseInt(textoSenha.match(/Senha (\\d+)/)[1], 10);
  const destino = textoSenha.split(\" - \")[1];
  ...
  if (!maioresSenhasPorColuna[idColuna] || numeroSenha > maioresSenhasPorColuna[idColuna].numero) {
...
function centralizarBotao(textoSenha) {
  ...
  // **Ajuste aqui:** coloque os IDs corretos das colunas do usuário 2.
  const colunas = ['coluna-normal', 'coluna-preferencial'];
  ...
  for (const idColuna of colunas) {
    ...
    for (const btn of botoes) {
      console.log(`Comparando: ${btn.textContent} com ${textoSenha}`);
      if (btn.textContent.includes(textoSenha)) {
        console.log(`Encontrado! Centralizando: ${btn.textContent}`);
        btn.scrollIntoView({
          behavior: 'smooth',
          block: 'center'
        });
  ...
  if (!encontrou) {
    console.log(`Botão ${textoSenha} ainda não existe. Tentando novamente em 200ms...`);
...
function escutarAlteracoesMaioresSenhas() {
  firebase.database().ref('maioresSenhasPorColuna').on('value', snapshot => {
    ...
    for (let idColuna in estado) {
      const limite = estado[idColuna].numero;
...
function salvarMaiorSenhaFirebase(idColuna, numeroSenha) {
  firebase.database().ref('maioresSenhasPorColuna/' + idColuna).set({
    numero: numeroSenha,
...
function criarBotao(idColuna, texto, classe) {
  ...
  texto: texto
    ...
    const limite = maioresSenhasPorColuna[idColuna]; // Sempre o maior já chamado
const classeDestaque = isPreferencial ? 'botao-destacado-preferencial' : 'botao-destacado-normal';

// Primeiro remove o destaque anterior
botoesNaColuna.forEach(btn => {
  btn.classList.remove('botao-destacado-normal', 'botao-destacado-preferencial');
});

// Sempre aplica o destaque até o MAIOR já chamado, independente do que foi clicado agora
botoesNaColuna.forEach(btn => {
  const match = btn.textContent.match(/Senha (\d+)/);
  if (match) {
    const num = parseInt(match[1], 10);
...
document.addEventListener("DOMContentLoaded", async function () {
  ...
  escutarUltimaSenha('normal');
  escutarUltimaSenha('preferencial');
});
...
async function chamarSenhaSincronizada(tipo, guiche) {
  ...
  const textoSenha = `Senha ${contador} - Guichê ${guiche}`;

  processarChamadaSenha(idColuna, textoSenha, tipo);
}
