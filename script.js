...
firebase.database().ref('senhaHover').on('value', (snapshot) => {
    const textoHover = snapshot.val();
    if (!textoHover || textoHover === ultimoHover) return;
    
    // Remove hover anterior
    if (ultimoHover) {
        document.querySelectorAll(`button[data-senha="${ultimoHover}"]`)
            .forEach(btn => btn.classList.remove('hover-ativo'));
    }
    
    // Aplica novo hover
    document.querySelectorAll(`button[data-senha="${textoHover}"]`)
        .forEach(btn => {
            btn.classList.add('hover-ativo');
            btn.scrollIntoView({behavior: 'smooth', block: 'center'});
        });
    
    ultimoHover = textoHover;
});
...
function escutarAlteracoesMaioresSenhas() {
  firebase.database().ref('maioresSenhasPorColuna').on('value', snapshot => {
    ...
    for (let idColuna in estado) {
      ...
      // Atualiza também a coluna sincronizada (guichê espelho)
...
function criarBotao(idColuna, texto, classe) {
  ...
  texto: texto
	...
	const senhaID = `S${isPreferencial ? 'P' : 'N'}${numeroSenha}`;
    botao.dataset.senha = senhaID;
    firebase.database().ref('senhaHover').set(senhaID);
  };
