...
function escutarAlteracoesMaioresSenhas() {
  firebase.database().ref('maioresSenhasPorColuna').on('value', snapshot => {
    ...
    for (let idColuna in estado) {
      ...
      destacarBotoes(idColuna, limite, classeDestaque);
      const colunaSincronizadaID = obterColunaSincronizada(idColuna);
      if (colunaSincronizadaID) {
        destacarBotoes(colunaSincronizadaID, limite, classeDestaque);
      }
    }
  });
}

function destacarBotoes(idColuna, limite, classeDestaque) {
  const coluna = document.getElementById(idColuna);
  if (!coluna) {
    console.warn(`Coluna ${idColuna} não encontrada, tentando novamente em 200ms...`);
    setTimeout(() => destacarBotoes(idColuna, limite, classeDestaque), 200);
    return;
  }

  const botoes = Array.from(coluna.querySelectorAll('button'));
  if (botoes.length === 0) {
    console.warn(`Nenhum botão na ${idColuna}, tentando novamente em 200ms...`);
    setTimeout(() => destacarBotoes(idColuna, limite, classeDestaque), 200);
    return;
  }
...
function criarBotao(idColuna, texto, classe) {
  ...
  const isPreferencial = idColuna.includes("preferencial");
  const match = texto.match(/Senha (\d+)/);
  const numeroSenha = match ? parseInt(match[1], 10) : 0;
...
if (maioresSenhasPorColuna[idColuna] && numeroSenha <= maioresSenhasPorColuna[idColuna]) {
  const classeDestaque = isPreferencial ? 'botao-destacado-preferencial' : 'botao-destacado-normal';
  botao.classList.add(classeDestaque);
}
...
function obterColunaSincronizada(idColuna) {
  ...
  if (idColuna === 'coluna-normal-posconsulta') return 'coluna-normal-posconsulta';
  ...
  if (idColuna === 'coluna-preferencial-posconsulta') return 'coluna-preferencial-posconsulta';
