...
firebase.database().ref('senhaHoverAtual').set({
  idColuna: idColuna,
  texto: texto
});


function escutarHoverGlobal() {
  firebase.database().ref('senhaHoverAtual').on('value', (snapshot) => {
    const data = snapshot.val();
    if (!data) return;

    const { idColuna, texto } = data;

    const todasColunas = document.querySelectorAll('.coluna');
    todasColunas.forEach(coluna => {
      coluna.querySelectorAll('button').forEach(btn => {
...
firebase.database().ref('senhaHoverAtual').on('value', (snapshot) => {
  ...
  // Remove hover de todos os botões
  const todasColunas = document.querySelectorAll('.coluna');
  todasColunas.forEach(coluna => {
    coluna.querySelectorAll('button').forEach(btn => {
      btn.classList.remove('hover-ativo');
  ...
  // Aplica hover no botão correspondente
  const coluna = document.getElementById(idColuna);
  if (!coluna) return;
